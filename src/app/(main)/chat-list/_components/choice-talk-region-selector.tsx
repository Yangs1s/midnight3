...
function ChoiceTalkRegionSelector({
  title,
  setTitle,
}: {
  title: string;
  ...
  const [value, setValue] = useState("지역 전체");
  const [isSearchOpen, setIsSearchOpen] = React.useState(false);

  return (
    <div className="relative py-2 flex w-full items-center gap-2">
      {isSearchOpen ? (
        <div className="flex flex-1 items-center gap-2">
          <div className="flex-1 bg-[#26252A] flex items-center rounded-[4px] px-2">
            <svg
            ...
            <Input
              className="flex-1 bg-[#26252A] border-none text-white placeholder:text-neutral-500 focus:outline-none focus:ring-0 h-[35px] outline-0"
          ...
          <Drawer open={open} onOpenChange={setOpen}>
            <DrawerTrigger asChild>
              <button
                type="button"
                className={cn(
                  "w-[120px] border border-white bg-transparent !px-[15px] !py-[8px] rounded-full text-white"
                )}
              >
            ...
            <DrawerContent>
              <DrawerHeader className="py-8 px-4">
                <DrawerTitle className="text-white text-lg">
                  <div className="flex items-center justify-between">
                    <p className="text-white text-lg">지역 선택</p>
              ...
              <div className="p-4 pt-0 -mx-4">
                {regions.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-4 border-b border-[#26252A] last:border-none flex items-center justify-between"
                    onClick={() => setValue(item.value)}
                  >
                    <p
                      className={`text-sm ${value === item.value ? "text-primary" : "text-[#C1C1C1]"}`}
                    >
                      {item.label}
                    </p>
                    {value === item.value && (
                      ...
                      >
                        <path
                          d="M17.2992 9.21094L10.1397 16.4109L7.69922 13.9567"
                          stroke="#985CFF"
                          strokeWidth="2"
                          strokeLinecap="round"
...
export default ChoiceTalkRegionSelector;
